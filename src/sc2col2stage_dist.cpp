...
static void
convert_to_sorted_distances(const Rcpp::NumericVector& distances,std::vector<Distance>& sorted_distances,int N) {
  sorted_distances.clear();
  sorted_distances.reserve(N * (N - 1) / 2);
  long long pos = 0; 
  for (int j = 0; j < N-1; ++j) {   
      for (int i = j+1; i < N; ++i) { 
       sorted_distances.push_back({j, i, distances[pos++]});
      }
  }
...
static void
convert_to_smallest_j_distances(const Rcpp::NumericVector& distances,std::vector<Distance>& sorted_distances,int N,size_t J) {
  ...
  size_t pos = 0; 
  const double* ptr = distances.begin();
...
// [[Rcpp::export]]
List solver_sc2col2stage_dist(const NumericVector& distances,
                               const int N,
  ...
  convert_to_smallest_j_distances(distances, sorted_distances, N, N); 
  ...
  if(!dispersion_found){
    ...
    convert_to_sorted_distances(distances, sorted_distances, N); 
    stage(sorted_distances, N, A, B, adj, last_adj, nodes, node_seen, color_fixated, last_color_fixated, dispersions_considered, dispersion_found, res);
  }
