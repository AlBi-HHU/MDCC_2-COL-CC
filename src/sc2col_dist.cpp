...
static void
convert_to_sorted_distances(const Rcpp::NumericVector& dvec,std::vector<Distance>& sorted_distances,int N) {
  size_t pos = 0; 
  sorted_distances.reserve(1LL * N * (N - 1) / 2);
  const double* ptr = dvec.begin();
  for (int i = 1; i < N; ++i) {      
      for (int j = 0; j < i; ++j) {  
       sorted_distances.push_back({j, i, ptr[pos++]});
      }
  }
...
// [[Rcpp::export]]
List solver_sc2col_dist(const NumericVector& distances,
                               const int N,
