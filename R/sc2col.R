library(Rcpp)
library(anticlust)
sourceCpp("src/sc2col_data.cpp")
sourceCpp("src/sc2col_dist.cpp")

optimal_dispersion_sc2col <- function(x, K = NULL) {
  ...
  if (is_dist) {
    result <- solver_sc2col_dist(x, N, target_groups)
  } else {
    result <- solver_sc2col_data(x, N, target_groups)
  }
}
